...
! perl utils/validate_dict_dir.pl $srcdir && \
...
if ! perl utils/validate_dict_dir.pl $srcdir >&/dev/null; then
  perl utils/validate_dict_dir.pl $srcdir  # show the output.
...
if $share_silence_phones; then
  ...
  cat $srcdir/{,non}silence_phones.txt | perl utils/apply_map.pl $tmpdir/phone_map.txt > $dir/phones/sets.txt
  cat $dir/phones/sets.txt | awk '{print "shared", "split", $0;}' > $dir/phones/roots.txt
fi

cat $srcdir/silence_phones.txt | perl utils/apply_map.pl $tmpdir/phone_map.txt | \
  awk '{for(n=1;n<=NF;n++) print $n;}' > $dir/phones/silence.txt
cat $srcdir/nonsilence_phones.txt | perl utils/apply_map.pl $tmpdir/phone_map.txt | \
...
cat $srcdir/extra_questions.txt 2>/dev/null | perl utils/apply_map.pl $tmpdir/phone_map.txt \
...
if "$silprob"; then
  ndisambig=$(perl utils/add_lex_disambig.pl $unk_opt --pron-probs --sil-probs $tmpdir/lexiconp_silprob.txt $tmpdir/lexiconp_silprob_disambig.txt)
else
  ndisambig=$(perl utils/add_lex_disambig.pl $unk_opt --pron-probs $tmpdir/lexiconp.txt $tmpdir/lexiconp_disambig.txt)
fi
...
cat $tmpdir/lexiconp.txt | awk '{print $1}' | python utils/sortuniq.py | awk '
  BEGIN {
    print "<eps> 0";
  }
  {
...
cat $tmpdir/align_lexicon.txt | \
  perl -ane '@A = split; print $A[0], " ", join(" ", @A), "\n";' | python utils/sortuniq.py > $dir/phones/align_lexicon.txt
...
if [ -f $srcdir/nonterminals.txt ]; then
  ...
  perl utils/sym2int.pl $dir/phones.txt <$dir/phones/nonterminals.txt >$dir/phones/nonterminals.int
  ...
  perl utils/sym2int.pl $dir/phones.txt <$dir/phones/left_context_phones.txt >$dir/phones/left_context_phones.int
...
cat $dir/phones/align_lexicon.txt | perl utils/sym2int.pl -f 3- $dir/phones.txt | \
  perl utils/sym2int.pl -f 1-2 $dir/words.txt > $dir/phones/align_lexicon.int
...
if $silprob; then
  ...
  python utils/lang/make_lexicon_fst_silprob.py $grammar_opts --sil-phone=$silphone \
         $tmpdir/lexiconp_silprob.txt $srcdir/silprob.txt | \
     fstcompile --isymbols=$dir/phones.txt --osymbols=$dir/words.txt \
       --keep_isymbols=false --keep_osymbols=false |   \
     fstarcsort --sort_type=olabel > $dir/L.fst || exit 1;
else
  python utils/lang/make_lexicon_fst.py $grammar_opts --sil-prob=$sil_prob --sil-phone=$silphone \
            $tmpdir/lexiconp.txt | \
    fstcompile --isymbols=$dir/phones.txt --osymbols=$dir/words.txt \
      --keep_isymbols=false --keep_osymbols=false | \
    fstarcsort --sort_type=olabel > $dir/L.fst || exit 1;
...
cat $dir/oov.txt | perl utils/sym2int.pl $dir/words.txt >$dir/oov.int || exit 1;
...
perl utils/sym2int.pl $dir/phones.txt <$dir/phones/wdisambig.txt >$dir/phones/wdisambig_phones.int
perl utils/sym2int.pl $dir/words.txt <$dir/phones/wdisambig.txt >$dir/phones/wdisambig_words.int
...
for f in silence nonsilence optional_silence disambig context_indep; do
  perl utils/sym2int.pl $dir/phones.txt <$dir/phones/$f.txt >$dir/phones/$f.int
  perl utils/sym2int.pl $dir/phones.txt <$dir/phones/$f.txt | \
   awk '{printf(":%d", $1);} END{printf "\n"}' | sed s/:// > $dir/phones/$f.csl || exit 1;
done

for x in sets extra_questions; do
  perl utils/sym2int.pl $dir/phones.txt <$dir/phones/$x.txt > $dir/phones/$x.int || exit 1;
done

perl utils/sym2int.pl -f 3- $dir/phones.txt <$dir/phones/roots.txt \
   > $dir/phones/roots.int || exit 1;

if [ -f $dir/phones/word_boundary.txt ]; then
  perl utils/sym2int.pl -f 1 $dir/phones.txt <$dir/phones/word_boundary.txt \
...
perl utils/gen_topo.pl $num_nonsil_states $num_sil_states $nonsilphonelist $silphonelist >$dir/topo
...
if $silprob; then
  ...
  python utils/lang/make_lexicon_fst.py $grammar_opts \
...
! perl utils/validate_lang.pl $dir && echo "$(basename $0): error validating output" &&  exit 1;
