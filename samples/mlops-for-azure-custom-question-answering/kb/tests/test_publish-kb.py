...
from kb.tests.fake_api import fakeQnAMakerAPI, fakeQnAMakerAPIreplaceCallBroken, fakeQnAMakerAPIsyncCallBroken,\
                              fakeQnAMakerAPIpublishCallBroken, returnSentBody, resetSentBody
import json
...
def test_for_branch_completeness(mocker: MockFixture):
    ...
    runpy.run_module('kb.scripts.publish-kb',
                     run_name='__something__')
...
def test_publish_sync(mocker: MockFixture):
    ...
    testArgs = Namespace(input='file.in', sync_feedback='y', sync_timespan='5d', publish_only=False)
...
def test_publish_wo_sync(mocker: MockFixture):
    ...
    resetSentBody()
    # Mocking cmd line arguments
    testArgs = Namespace(input='file.in', sync_feedback='n', sync_timespan='5d', publish_only=False)
    ...
    assert returnSentBody(0) == json.dumps({"qnAList": jsonKBwOALFeedback['qnaDocuments']})
...
def test_publish_error_on_publish(mocker: MockFixture):
    ...
    """
    # Mocking cmd line arguments
    testArgs = Namespace(input='file.in', sync_feedback='n', sync_timespan='5d', publish_only=True)
