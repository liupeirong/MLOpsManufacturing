...
def test_run_data_processing_pipeline_happy_path(mocker: MockFixture,
    ...
    mock_args.return_value = edict({"aml_pipeline_name": "aml_pipeline_name_test", "output_pipeline_id_file": "w", "skip_preprocessing_execution": False})

    mock_aml_pipelines_list = mocker.patch('azureml.pipeline.core.PublishedPipeline.list')
    type(mock_pipeline_publish).name = mocker.PropertyMock(return_value='aml_pipeline_name_test')
